...
export function ApprovalIntegrationPage() {
  ...
  function submitApprovalAction(task: PendingTask, type: 'approve' | 'reject', comment: string) {
  ...
  return (
    <section className="approval-v6">
      ...
      <main className="approval-v6__main">
        <header className="approval-v6__topbar">
          <span>数据资产平台</span><span>/</span>
          <span>{section === 'flows' || section === 'flow-detail' || section === 'roles' ? '配置管理' : section === 'monitor' ? '运维监控' : '审批工作台'}</span>
          <span>/</span><strong>{section === 'flow-detail' ? '流程详情' : navGroups.flatMap(g => g.items).find(item => item.key === section)?.label}</strong>
          <em>当前用户：刘数据（数据分析部）</em>
        </header>
        {toast ? <div className="approval-v6__toast" role="status">{toast}</div> : null}
        <div className="approval-v6__content">
          {section === 'flows' ? <FlowsPanel flows={flows} routes={routes} validatingId={validatingId} onNew={() => setDrawer({ kind: 'new-flow', flow: newFlow() })} onEdit={openFlowDetail} onValidate={validateFlow} onToggle={toggleFlowStatus} onOpenRoutes={(ticketType) => setDrawer({ kind: 'route', route: newRouteFor(ticketType, flows, routes), isNew: true })} /> : null}
          ...
          {section === 'pending' ? <PendingPanel tasks={tasks} onOpenAction={setActionDialog} onFeishu={() => flash('跳转飞书审批（演示）')} /> : null}
...
function FlowsPanel({ flows, routes, validatingId, onNew, onEdit, onValidate, onToggle, onOpenRoutes }: { flows: FlowConfig[]; routes: FlowRoute[]; validatingId: string | null; onNew: () => void; onEdit: (id: string) => void; onValidate: (id: string) => void; onToggle: (id: string) => void; onOpenRoutes: (ticketType: string) => void }) {
  ...
  const flowStats = useMemo(() => {
    const complete = flows.filter(flow => flow.formMappingStatus === 'complete' && flow.nodeMappingStatus === 'complete').length;
    const healthy = flows.filter(flow => flowHealth(flow).label === '健康').length;
    const attention = flows.filter(flow => ['异常', '待校验', '待配置'].includes(flowHealth(flow).label)).length;
    const enabledRoutes = routes.filter(route => route.enabled).length;
    return {
      total: flows.length,
      enabled: flows.filter(flow => flow.status === 'enabled').length,
      completeRate: flows.length ? Math.round((complete / flows.length) * 100) : 0,
      healthy,
      attention,
      enabledRoutes,
    };
  ...
  const grouped = useMemo(() => {
    ...
    return ticketTypes.map(type => ({ type, flows: filtered.filter(flow => flow.ticketType === type), routes: routes.filter(route => route.ticketType === type).sort((a, b) => a.priority - b.priority) }));
  ...
  return (
    <>
      <div className="approval-v6__page-header approval-v6__page-header--ops">
        <div>
          <small className="approval-v6__eyebrow">配置管理 / 流程运营</small>
          <h1>飞书流程库</h1>
          <p>集中运营工单类型与飞书审批定义的绑定关系，按路由规则命中流程并监控配置健康度。</p>
        </div>
        <div className="approval-v6__header-actions">
          <span>最近同步 2026-06-11 14:20</span>
          <Button variant="primary" onClick={onNew}>新增流程</Button>
        </div>
      </div>
      <div className="approval-v6__ops-cards">
        <div><span>流程定义</span><strong>{flowStats.total}</strong><small>{flowStats.enabled} 个启用中</small></div>
        <div><span>配置完整率</span><strong>{flowStats.completeRate}%</strong><small>字段映射 + 节点映射</small></div>
        <div><span>路由规则</span><strong>{flowStats.enabledRoutes}</strong><small>启用规则数</small></div>
        <div className={flowStats.attention ? 'warning' : 'success'}><span>待治理项</span><strong>{flowStats.attention}</strong><small>{flowStats.healthy} 个健康运行</small></div>
      </div>
        ...
        {grouped.map(group => (
          <section key={group.type} className="approval-v6__flow-group">
            <button type="button" className="approval-v6__flow-group-head" onClick={() => setExpandedTypes(prev => { const next = new Set(prev); next.has(group.type) ? next.delete(group.type) : next.add(group.type); return next; })}>
              ...
              {group.routes.length ? <small className="route-count">{group.routes.length} 条路由规则</small> : null}
              <em onClick={(event) => { event.stopPropagation(); onOpenRoutes(group.type); }}>路由规则</em>
            </button>
            {expandedTypes.has(group.type) && group.routes.length ? <RoutePreview routes={group.routes} /> : null}
            {expandedTypes.has(group.type) ? <FlowTable flows={group.flows} validatingId={validatingId} onEdit={onEdit} onValidate={onValidate} onToggle={onToggle} /> : null}
          </section>
        ))}
      </div>
      <div className="approval-v6__note"><strong>集成说明：</strong>飞书侧负责配置审批流程、节点顺序、条件分支和会签规则。平台维护 <b>approval_code 绑定、表单字段映射、动态审批节点映射和路由规则</b>。提交时按路由规则优先级匹配，命中第一条即走对应流程。</div>
    </>
  );
}

function RoutePreview({ routes }: { routes: FlowRoute[] }) {
  return <div className="approval-v6__route-preview"><b>路由命中顺序</b>{routes.map((route, index) => <span key={route.id} className={route.isDefault ? 'fallback' : ''}><i>{index + 1}</i>{route.isDefault ? '兜底' : `P${route.priority}`} {route.name}</span>)}</div>;
}

function FlowTable({ flows, validatingId, onEdit, onValidate, onToggle }: { flows: FlowConfig[]; validatingId: string | null; onEdit: (id: string) => void; onValidate: (id: string) => void; onToggle: (id: string) => void }) {
  if (!flows.length) return <div className="approval-v6__empty">该工单类型暂无流程配置，点击新增</div>;
  return (
    <div className="approval-v6__table-wrap">
      <table>
        <thead><tr><th>飞书审批定义</th><th>责任团队</th><th>approval_code</th><th>运行状态</th><th>配置健康</th><th>最近校验</th><th>操作</th></tr></thead>
        <tbody>
          {flows.map(flow => {
            ...
            return (
              <tr key={flow.id}>
                ...
                <td><strong className="approval-v6__owner-cell">{flowOwnerLabel(flow.ticketType)}</strong><span>{flow.ticketType}</span></td>
...
function InstanceDrawer({ instance, onClose }: { instance: ApprovalInstance; onClose: () => void }) {
  ...
  return (
    <div className="approval-v6__drawer-mask" onClick={onClose}>
      <aside className="approval-v6__drawer" onClick={event => event.stopPropagation()}>
        <header>
          <h2>工单详情</h2>
        ...
        <div className="approval-v6__instance-detail">
          <div className="approval-v6__instance-header">
            ...
            <a href={instance.feishuUrl} target="_blank" rel="noreferrer">飞书查看</a>
          </div>

          <h3>申请信息</h3>
          <div className="approval-v6__instance-section">
            {/* 通用字段 */}
            <InfoItem label="申请资产" value={instance.assets.join('、')} />
            <InfoItem label="安全等级" value={instance.securityLevel} />
            <InfoItem label="目录路径" value={instance.directory} />
            <InfoItem label="来源" value={`${sourceTypeLabel(instance.sourceType)} / ${instance.sourceSystem}`} />
            <InfoItem label="命中流程" value={instance.matchedFlow} />
            <InfoItem label="命中规则" value={instance.matchedRoute} />
            <InfoItem label="申请原因" value={instance.reason} />

            {/* 权限申请特有 */}
            {ticketType === '权限申请' && (
              <>
                <InfoItem label="权限类型" value={instance.permissionType} />
                <InfoItem label="申请期限" value={instance.expireDate} />
              </>
            )}
            {/* 上架审批特有 */}
            {ticketType === '上架审批' && (
              <InfoItem label="上架说明" value={instance.permissionType || '—'} />
            )}
            {/* 下架审批特有 */}
            {ticketType === '下架审批' && (
              <>
                <InfoItem label="下架原因" value={instance.permissionType || '—'} />
                <InfoItem label="影响使用方" value={instance.assets.length > 0 ? '见资产列表' : '—'} />
              </>
            )}
            {/* 目录修改特有 */}
            {ticketType === '目录修改' && (
              <>
                <InfoItem label="变更类型" value="拖动目录" />
                <InfoItem label="原目录" value={instance.directory} />
              </>
            )}
            {/* 负责人交接特有 */}
            {ticketType === '负责人交接' && (
              <InfoItem label="交接类型" value={instance.permissionType || '—'} />
            )}
            {/* 血缘修正特有 */}
            {ticketType === '血缘修正' && (
              <InfoItem label="变更说明" value={instance.permissionType || '—'} />
            )}
          </div>

          <h3>申请资产明细</h3>
          <div className="approval-v6__instance-assets">
            <table className="approval-v6__assets-table">
              <thead>
                <tr>
                  <th>资产名称</th>
                  <th>类型</th>
