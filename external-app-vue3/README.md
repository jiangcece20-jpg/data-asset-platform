...
列表只留三槽，由字段派生，不把 `tags` 整排铺上去。权威口径见功能说明 PRD **§10.1.5**；实现见 `src/domain/productListChips.ts`。

### 三槽（列表 / 找数 / 搜索）

| 槽 | 必出 | 规则 |
|----|------|------|
| 商品类型 | 是 | 数据集 / API / 行业报告 / 自有看板（`product.type`） |
| 成交位置 | 否 | `dealChannel === space_purchase` → 空间名；`origin === seller_market` → 卖家名；APP 自营 / 资产平台 → **空**（筛选里叫「本平台」）。不写「可信空间购买」「APP 支付」「空间商品」「入驻商家」 |
| 运营标签 | 否 | 最多一颗。异常状态优先：可申请上架 / 准备中 / 暂停销售 / 已下架。活动类互斥，优先级：**合规首选 > 个人数据集 > 热门**（`recommendSlot` 或 tags 含「热门」「热门数据集」） |

### 不进列表

- 样例 / 试用：数据集 `hasSampleData` →「有样例」；API `hasTrialApi` →「有试用接口」——仅详情 `ProductChips showTrial`
- 渠道、来源、会员价、免费——写在价格区或详情其它区块

### 话题标签（详情标题下）

`productTopicTags` = `product.tags` 去掉事实/活动复读集后的剩余项（如「合规核验」「港口吞吐量」）。复读集含：空间商品、数据集、API、入驻商家、资产平台、热门、热门数据集、会员免费、免费、合规首选、个人数据集、个人可购 等——见 `FACT_OR_CAMPAIGN_TAGS`。

### 筛选

两个下拉与槽 1、2 对齐：全部类型 / 全部位置（本平台、空间名、卖家名、入驻商家）。运营角标只出现在卡片上，第一阶段不进筛选。

空间商品必须有空间名，缺了会看起来像自营。
...
2. `/#/admin/orders`：空间购买意向单与 APP 订单统一在订单中心展示，交易状态为「意向单」；运营领取后转为买数订单。确认企业、确认方案、线下试用都在线下完成，系统不增加这些节点。「去空间处理」只用于协调履约。数据集接入后进「我的数据」；API 开通后订单完成并给使用说明。
...
- 会员三种互斥：个人会员（万联灵析个人版，1099 元/年）、团队会员（万联灵析企业版，3199 元/年）、组合会员（商业化全产品版，原型暂不从商品详情售卖）。个人身份只生效个人/组合会员，企业身份只生效当前企业团队/组合会员；进入企业身份后个人会员立即作废，已有团队会员不能再买个人会员。列表只展示当前身份价格摘要（如 `¥199 起 · 会员免费`），不出交易按钮；详情展示「价格 / 会员价」+「立即购买」，点击进结算页再选「直接购买 / 成为会员」及立省气泡。演示：`/#/app/search` 看列表价摘要 → `/#/app/product/prod-freight-index` 看价格区 → 点「立即购买」进结算双路径。
