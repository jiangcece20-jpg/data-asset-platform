...
| 空间数据集 | APP 提交意向单，运营到空间代办成交 | 有无样例标签（未登录可看样例） | 数据集接到本平台用数 |
| API | APP 提交意向单，运营到空间代办成交 | 有无试用接口标签（本平台不真调） | 仍在空间使用；本平台只记录结果 |
...
空间负责成交、合同和付款；运营代办。本平台负责找数、看样例、收意向、盯数据集交付。数据集接到本平台用数；API 仍在空间使用。自有空间名称「万联易达可信空间」。APP 不展示自有/互联。
